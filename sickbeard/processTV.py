...
def processDir(dirName, nzbName=None, process_method=None, force=False, is_priority=None, ignore_subs=None, delete_on=False, failed=False, proc_type="auto"):
    """
...
def process_media(processPath, videoFiles, nzbName, process_method, force, is_priority, ignore_subs, result):  # pylint: disable=too-many-arguments
    ...
    for cur_video_file in videoFiles:
        ...
        try:
            ...
            if sickbeard.POSTPONE_IF_NO_SUBS and subtitlesEnabled(cur_video_file) and not ignore_subs:
...
def subtitlesEnabled(video):
    try:
        parse_result = NameParser().parse(video, cache_result=False)
